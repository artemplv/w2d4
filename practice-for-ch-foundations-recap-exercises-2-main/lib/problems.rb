...
def least_common_multiple(num_1, num_2)
  product = num_1 * num_2
  ([num_1, num_2].min...product).each do |num|
...
def most_frequent_bigram(str)
  ...
  (0...str.length - 1).each do |i|
    bigrams.push(str[i..i + 1])
  end

  freq = bigrams.inject(Hash.new(0)) do |hash, value|
    hash[value] += 1
    hash
  end

  bigrams.max_by { |value| freq[value] }
end
  ...
  def pair_sum_count(num)
    ...
    (0...self.length - 1).each do |i|
      (i + 1...self.length).each do |j|
        sums.push(self[i] + self[j])
      end
    end

    sums.count { |sum| sum == num }
  end
